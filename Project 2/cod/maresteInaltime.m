function img = maresteInaltime(img,numarPixeliLatime,metodaSelectareDrum,ploteazaDrum,culoareDrum)
...
Masca = zeros(size(img,1),size(img,2));
for i = 1:numarPixeliLatime
    ...
    E(:,:) = E(:,:)+Masca(:,:);
    drum(:,:,i) = selecteazaDrumVertical(E,metodaSelectareDrum);
    %afiseaza drum
    for j = 1:size(img,1)
    Masca(drum(j,1,i),drum(j,2,i))= 100000;
    end
    if ploteazaDrum
...
for i = 1:numarPixeliLatime
img = adaugaDrumVertical(img,drum(:,:,i));
end
img = permute(img,[2 1 3]);